    ...
    HTLCPaymentCapacity
{
    ...
    constructor(string memory name_, address owner) 
        // WhitelistedSenders(owner)
        // HTLCPaymentCapacity(owner)
    {
    ...
    function transfer(
        ...
        return
            _bank().transfer(
                RecipentInfo(ISoCashAccount(to), BIC.wrap(0), IBAN.wrap(0)),
                amount,
                "ERC20 Transfer"
            );
    }
    ...
    function transferEx(
        RecipentInfo calldata recipient,
        uint256 amount,
    ...
    function _transferUsingAllowance(
        address sender,
    ...
    ) public onlyWhitelisted returns (bytes32 key) {
        key = saveHTLCPayment(
            recipient,
            amount,
            deadline,
